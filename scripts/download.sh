...
# Define variables
DOCS_REPO_URL="https://github.com/dynamsoft-docs/batch-barcode-scanner-docs/archive/refs/heads/preview.zip"
...
FINAL_DIR="batch-barcode-scanner-docs-preview"
...
# Download first repository (batch-barcode-scanner-docs)
echo "Downloading batch-barcode-scanner-docs repository..."
curl -L -o docs.zip "$DOCS_REPO_URL"

# Extract first repository
echo "Extracting batch-barcode-scanner-docs..."
unzip -q docs.zip

# Get the extracted directory name (dynamic detection)
DOCS_EXTRACTED_DIR=$(find . -maxdepth 1 -type d -name "batch-barcode-scanner-docs-*" | head -1 | sed 's|^\./||')
